...
server.registerTool("generate_person", {
...
server.registerTool("generate_people", {
...
server.registerTool("generate_title", {
...
server.registerTool("generate_phone_number", {
...
server.registerTool("generate_bvn", {
...
server.registerTool("generate_nin", {
...
server.registerTool("generate_vehicle_record", {
  title: "Generate a fake vehicle record using naija-faker tool",
  description: "Generates a fake vehicle record data. Accept a payload of string for the state of the preferred vehicle. Returns the processed result as an object",
  inputSchema: {
...
server.registerTool("generate_license_plate", {
  title: "Generate a fake license plate using naija-faker tool",
  description: "Generates a fake license plate data. Accept a payload of string for the state of the preferred license plate. Returns the processed result as a string",
  inputSchema: {
    state: z.string().optional().describe("The state of the preferred license plate"),
  }
}, async ({ state }) => {
  try {
...
server.registerTool("generate_company", {
  ...
  description: "Generates a fake company data. Returns the processed result as an object",
}, async () => {
  try {
...
server.registerTool("generate_university", {
  title: "Generate a fake university using naija-faker tool",
  description: "Generates a fake university data. Returns the processed result as an object",
}, async () => {
  try {
...
server.registerTool("generate_education_record", {
  title: "Generate a fake education record using naija-faker tool",
  description: "Generates a fake education record data. Accept a payload of string for the language of the preferred education record. Returns the processed result as an object",
  inputSchema: {
    language: z.string().optional().describe("The language of the education record. The available languages are Hausa, Igbo and Yoruba"),
  }
}, async ({ language }) => {
  try {
    const educationRecord = faker.educationRecord(language as "hausa" | "igbo" | "yoruba");
    return {
      content: [{ type: "text", text: JSON.stringify(educationRecord, null, 2) }],
    };
  } catch (error) {
    return {
      content: [{ type: "text", text: `Error: ${error}` }],
...
server.registerTool("generate_work_record", {
  title: "Generate a fake work record using naija-faker tool",
  description: "Generates a fake work record data. Returns the processed result as an object",
}, async () => {
  try {
...
server.registerTool("generate_detailed_person", {
  title: "Generate a fake detailed person using naija-faker tool",
  description: "Generates a fake detailed person data. Accepts a payload of string for the language of the preferred detailed person and a string payload for the gender of the detailed person. Returns the processed result as an object",
  inputSchema: {
    language: z.string().optional().describe("The language of the detailed person. The available languages are Hausa, Igbo and Yoruba"),
    gender: z.string().optional().describe("The gender of the detailed person. The accepted gender values are male and female"),
  }
}, async ({ language, gender }) => {
  try {
    const detailedPerson = faker.detailedPerson(language as "hausa" | "igbo" | "yoruba", gender as "male" | "female");
    return {
      content: [{ type: "text", text: JSON.stringify(detailedPerson, null, 2) }],
    };
  } catch (error) {
    return {
      content: [{ type: "text", text: `Error: ${error}` }],
...
server.registerTool("generate_detailed_people", {
  title: "Generate a fake detailed people using naija-faker tool",
  description: "Generates a fake detailed people data. Accepts a payload of string for the language of the preferred detailed people and a string payload for the gender of the detailed people. Returns the processed result as an object",
  inputSchema: {
    language: z.string().optional().describe("The language of the detailed people. The available languages are Hausa, Igbo and Yoruba"),
    gender: z.string().optional().describe("The gender of the detailed people. The accepted gender values are male and female"),
    count: z.number().optional().describe("The number of detailed people to generate"),
  }
}, async ({ language, gender, count }) => {
  try {
    const detailedPeople = faker.detailedPeople(count as number || 1, language as "hausa" | "igbo" | "yoruba", gender as "male" | "female");
    return {
      content: [{ type: "text", text: JSON.stringify(detailedPeople, null, 2) }],
    };
  } catch (error) {
    return {
      content: [{ type: "text", text: `Error: ${error}` }],
...
server.registerTool("generate_date_of_birth", {
  title: "Generate a fake date of birth using naija-faker tool",
  description: "Generates a fake date of birth data. Returns the processed result as an object",
  inputSchema: {
    minAge: z.number().optional().describe("The minimum age of the date of birth"),
    maxAge: z.number().optional().describe("The maximum age of the date of birth"),
  }
}, async ({ minAge, maxAge }) => {
  try {
    const dateOfBirth = faker.dateOfBirth({ minAge: minAge as number || 1, maxAge: maxAge as number || 100 });
    return {
...
server.registerTool("generate_marital_status", {
  title: "Generate a fake marital status using naija-faker tool",
  description: "Generates a fake marital status data. Returns the processed result as a string",
}, async () => {
  try {
...
server.registerTool("generate_blood_group", {
  title: "Generate a fake blood group using naija-faker tool",
  description: "Generates a fake blood group data. Returns the processed result as a string",
}, async () => {
  try {
...
server.registerTool("generate_genotype", {
  title: "Generate a fake genotype using naija-faker tool",
  description: "Generates a fake genotype data. Returns the processed result as a string",
}, async () => {
  try {
...
server.registerTool("generate_salary", {
  title: "Generate a fake salary using naija-faker tool",
  description: "Generates a fake salary data. Returns the processed result as a string",
  inputSchema: {
    level: z.string().optional().describe("The level of the salary. The accepted level values are entry, mid, executive and senior"),
  }
}, async ({ level }) => {
  try {
    const salary = faker.salary({ level: level as "entry" | "mid" | "executive" | "senior" });
    return {
...
server.registerTool("generate_next_of_kin", {
  title: "Generate a fake next of kin using naija-faker tool",
  description: "Generates a fake next of kin data. Accepts a payload of string for the language of the preferred next of kin and a string payload for the gender of the next of kin. Returns the processed result as an object",
  inputSchema: {
    language: z.string().optional().describe("The language of the next of kin. The available languages are Hausa, Igbo and Yoruba"),
    gender: z.string().optional().describe("The gender of the next of kin. The accepted gender values are male and female"),
  }
}, async ({ language, gender }) => {
  try {
    const nextOfKin = faker.nextOfKin(language as "hausa" | "igbo" | "yoruba", gender as "male" | "female");
    return {
...
server.registerTool("generate_states", {
  title: "Generate a list of states using naija-faker tool",
  description: "Generates a list of states data. Returns the processed result as an array",
}, async () => {
  try {
...
server.registerTool("generate_lgas", {
  title: "Generate a list of LGAs using naija-faker tool",
  description: "Generates a list of LGAs data. Returns the processed result as an array",
}, async () => {
  try {
...
server.registerTool("genereate_bank_account", {
  title: "Generate a fake bank account using naija-faker tool",
  description: "Generates a fake bank account data. Accepts a string payload for the bank name. Returns the processed result as an object",
  inputSchema: {
...
server.registerTool("generate_consistent_person", {
  title: "Generate a consistent fake person using naija-faker tool",
  description: "Generates a consistent fake person data where name ethincity, address, state and LGA are all geographically coherent. Accepts a string payload for the language of the person data and a string payload for the gender of the person data. Returns the processed result as an object",
  inputSchema: {
    language: z.string().optional().describe("The language of the person data. The available languages are Hausa, Igbo and Yoruba"),
    gender: z.string().optional().describe("The gender of the person data. The accepted gender values are male and female"),
  }
  ...
  try {
    const consistentPerson = faker.consistentPerson(language as "hausa" | "igbo" | "yoruba", gender as "male" | "female");
    return {
      content: [{ type: "text", text: JSON.stringify(consistentPerson, null, 2) }],
    };
  } catch (error) {
    return {
      content: [{ type: "text", text: `Error: ${error}` }],
...
server.registerTool("generate_consistent_people", {
  title: "Generate a list of consistent fake people using naija-faker tool",
  description: "Generates a list of consistent fake people data where name ethincity, address, state and LGA are all geographically coherent. Accepts a string payload for the language of the person data and a string payload for the gender of the person data. Returns the processed result as an array",
  inputSchema: {
    language: z.string().optional().describe("The language of the person data. The available languages are Hausa, Igbo and Yoruba"),
    gender: z.string().optional().describe("The gender of the person data. The accepted gender values are male and female"),
  ...
  try {
    const consistentPeople = faker.consistentPeople(count as number, language as "hausa" | "igbo" | "yoruba", gender as "male" | "female");
    return {
      content: [{ type: "text", text: JSON.stringify(consistentPeople, null, 2) }],
    };
  } catch (error) {
    return {
      content: [{ type: "text", text: `Error: ${error}` }],
...
server.registerPrompt("generate_person", {
  title: "Generates a fake person data using naija-faker tool",
